...
function ClinicalNotesPage() {
  ...
  useEffect(() => {
    ...
    const fetchPatients = async () => {
      try {
        const patientsRes = await fetch(`${backendUrl}${getApiPrefix()}/patients`, { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  useEffect(() => {
    ...
    const fetchPatientAndNotes = async () => {
      ...
      try {
        ...
        const patientRes = await fetch(`${backendUrl}${getApiPrefix()}/patients/${selectedPatientId}`, { headers: { 'Authorization': `Bearer ${token}` } });
        if (!patientRes.ok) {
        ...
        const notesRes = await fetch(`${backendUrl}${getApiPrefix()}/clinical-notes/patient/${selectedPatientId}`, {
    ...
    const fetchAppointments = async () => {
      if (showNoteModal && selectedPatientId && token) {
        try {
          ...
          const response = await fetch(`${backendUrl}${getApiPrefix()}/appointments?patient_id=${selectedPatientId}`, {
            headers: { 'Authorization': `Bearer ${token}` }
          });
          if (!response.ok) {
  ...
  const handleAddEditSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingNote
        ? `${backendUrl}${getApiPrefix()}/clinical-notes/${editingNote.id}`
        : `${backendUrl}${getApiPrefix()}/clinical-notes`;
      ...
      const updatedNotesRes = await fetch(`${backendUrl}${getApiPrefix()}/clinical-notes/patient/${selectedPatientId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDeleteNote = async (noteId) => {
    ...
    setNotification({ message: null, type: null });
    try {
      const response = await fetch(`${backendUrl}${getApiPrefix()}/clinical-notes/${noteId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
