...
function ScheduleManagementPage() {
    ...
    const fetchSchedules = useCallback(async () => {
        if (!isAuthenticated || authLoading) return;
        try {
            const response = await axios.get(`${backendUrl}/api/v1/schedules`, config);
    ...
    useEffect(() => {
        ...
        fetchSchedules();
        fetchData(`${backendUrl}/api/v1/schedules/availability`, setAvailabilities, 'fetching availabilities');
    ...
    const handleScheduleSubmit = async (e) => {
        ...
        try {
            ...
            const url = editingSchedule ? `${backendUrl}/api/v1/schedules/${editingSchedule.id}` : `${backendUrl}/api/v1/schedules`;
    ...
    const handleScheduleDelete = async () => {
        ...
        setNotification({ message: null, type: null });
        try {
            await axios.delete(`${backendUrl}/api/v1/schedules/${editingSchedule.id}`, config);
    ...
    const handleAvailabilitySubmit = async (e) => {
        ...
        try {
            ...
            if (editingAvailability) {
                ...
                await axios.put(`${backendUrl}/api/v1/schedules/availability/${editingAvailability.id}`, payload, config);
                ...
                for (const dayId of availabilityFormData.day_of_week) {
                    ...
                    await axios.post(`${backendUrl}/api/v1/schedules/availability`, payload, config);
                }
            ...
            resetAvailabilityFormData();
            fetchData(`${backendUrl}/api/v1/schedules/availability`, setAvailabilities, 're-fetching availabilities');
        } catch (error) {
    ...
    const handleDeleteAvailability = async (idToDelete) => {
        ...
        setNotification({ message: null, type: null });
        try {
            await axios.delete(`${backendUrl}/api/v1/schedules/availability/${idToDelete}`, config);
